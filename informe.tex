...
\usepackage{listings}
\usepackage{tikz}
% Se agregó la librería 'babel' para evitar errores con las flechas en español
...
% Trello table colors
\definecolor{trellohdr}{RGB}{0,82,147}
\definecolor{trellogrp}{RGB}{38,132,255}
\definecolor{trelloalt}{RGB}{235,244,255}
\definecolor{trellofoot}{RGB}{220,220,220}
...
\begin{titlepage}
  ...
  \begin{tabular}{ll}
    ...
    Benjamín De La Fuente      & \texttt{bdelafuente2025@alu.uct.cl}    \\
    \bottomrule
  \end{tabular}
  \vfill
...
En el mercado de vehículos usados, tanto compradores como vendedores enfrentan una fricción
...
\begin{itemize}
  ...
  \item \textbf{Chat}: los participantes de un match (comprador y vendedor del listing asociado)
        pueden enviar y listar mensajes. La autorización se verifica a nivel de servicio.
...
abstracciones (interfaces). Las implementaciones concretas tampoco dependen de los servicios; ambas dependen de las interfaces definidas en el paquete \texttt{repository} y \texttt{auth}. El \textbf{Composition Root} en \texttt{main.go} es el único lugar donde se instancian las
...
operan con las interfaces de repositorio definidas en el paquete \texttt{repository}. Cada agregado de dominio tiene su propia interfaz con los métodos estrictamente necesarios para
...
contraseña en texto plano y \texttt{Verify()} para compararla contra un hash almacenado. \texttt{AuthService} recibe un \texttt{PasswordHasher} en su constructor y nunca instancia
...
El backend expone una API REST bajo el prefijo \texttt{/api/v1} usando el framework Gin 1.12. La documentación interactiva completa está disponible en
...
\texttt{seller}), \texttt{iat} (emitido en) y \texttt{exp} (expira en, 24 horas tras la emisión). El middleware \texttt{JWTAuth} valida la firma con HS256 y rechaza tokens expirados o
...
% ===== 9. PLANIFICACIÓN Y TIEMPOS =====
\clearpage
\section{Planificación y tiempos del proyecto}

La siguiente tabla organiza las tareas del proyecto al estilo Trello, indicando el tiempo
estimado según la complejidad observada en el código y el tiempo real invertido por el equipo. Los tiempos están expresados en horas.

\begin{center}
\renewcommand{\arraystretch}{1.45}
\begin{longtable}{>{\raggedright\arraybackslash}p{8cm}
                  >{\centering\arraybackslash}p{2.8cm}
                  >{\centering\arraybackslash}p{2.8cm}}

  % ---- Encabezado principal ----
  \rowcolor{trellohdr}
  \textcolor{white}{\textbf{\sffamily Tarea}} &
  \textcolor{white}{\textbf{\sffamily Tiempo estimado}} &
  \textcolor{white}{\textbf{\sffamily Tiempo real}} \\
  \endfirsthead
  ...
  % ==============================
  \rowcolor{trellogrp}
  \multicolumn{3}{l}{\textcolor{white}{\textbf{\sffamily Planificación y diseño}}} \\

  Definición de requerimientos funcionales y diseño de la arquitectura del sistema
    & 3 h &  2 h \\
  \rowcolor{trelloalt}
  Diseño del modelo de datos (tablas, relaciones, migraciones SQL)
    & 3 h & 3 h \\
  ...
  % ==============================
  \rowcolor{trellogrp}
  \multicolumn{3}{l}{\textcolor{white}{\textbf{\sffamily Backend (Go / Gin)}}} \\

  Inicialización del monorepo y configuración de Docker Compose
    & 2 h & 1 h \\
  \rowcolor{trelloalt}
  Implementación de la capa de dominio: entidades y constructores con validaciones
    & 5 h & 4 h \\
  Implementación de repositorios PostgreSQL y migraciones SQL
    & 6 h & 5 h \\
  \rowcolor{trelloalt}
  Módulo de autenticación: registro, login, JWT (HS256) y hashing bcrypt
    & 4 h & 3 h \\
  Módulo de gestión de perfiles de comprador (preferencias de búsqueda)
    & 2 h & 1 h \\
  \rowcolor{trelloalt}
  Módulo de listings: CRUD completo con carga de fotos (N+1 avoidance)
    & 5 h & 4 h \\
  Módulo de feed: filtrado dinámico por preferencias del perfil del comprador
    & 3 h & 3 h \\
  \rowcolor{trelloalt}
  Módulo de swipe con match automático al hacer like y chat por match
    & 5 h & 4 h \\
  Configuración de CORS y generación de documentación Swagger
    & 2 h & 1 h \\
  ...
  % ==============================
  \rowcolor{trellogrp}
  \multicolumn{3}{l}{\textcolor{white}{\textbf{\sffamily Frontend (Next.js / React / TypeScript)}}} \\

  Inicialización del proyecto con Next.js 16, TypeScript y Tailwind CSS
    & 1 h & 2 h \\
  \rowcolor{trelloalt}
  Sistema de autenticación: AuthContext, ProtectedRoute y hooks JWT
    & 4 h & 5 h \\
  Páginas de registro e inicio de sesión
    & 2 h & 2 h \\
  \rowcolor{trelloalt}
  Página de feed con mecanismo de swipe y notificación de match
    & 4 h & 5 h \\
  Páginas de matches y chat con polling HTTP cada 3 segundos
    & 4 h & 6 h \\
  \rowcolor{trelloalt}
  Páginas de listings: listado, nueva publicación y edición
    & 4 h & 5 h \\
  Páginas de perfil y edición de preferencias de búsqueda
    & 2 h & 2 h \\
  \rowcolor{trelloalt}
  Componente PhotoUpload con vista previa y soporte multi-archivo
    & 2 h & 4 h \\

  % ==============================
  % GRUPO: Integración y cierre
  % ==============================
  \rowcolor{trellogrp}
  \multicolumn{3}{l}{\textcolor{white}{\textbf{\sffamily Integración y cierre}}} \\

  Corrección de bugs de integración frontend-backend (CORS, contratos API, polling)
    & 4 h & 6 h \\
  \rowcolor{trelloalt}
  ...
  % ==============================
  \rowcolor{trellofoot}
  \textbf{Total} & \textbf{71 h} & \textbf{72 h} \\
